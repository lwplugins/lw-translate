 ...
 */
import TranslateMark from '../components/TranslateMark';
		...
		<aside className={ `lw-admin-sidebar ${ isOpen ? 'is-open' : '' }` }>
			<div className="lw-admin-sidebar__head">
				<a
					...
					href={ `#${ tabs[ 0 ].id }` }
					aria-label={ __( 'LW Translate home', 'lw-translate' ) }
				>
					<TranslateMark />
					<strong>LW Translate</strong>
				</a>
			...
			<nav
				...
				aria-label={ __( 'LW Translate sections', 'lw-translate' ) }
			>
				<ul>
					{ tabs.map( ( tab ) => {
			...
			<div className="lw-admin-sidebar__foot">
				...
				>
					...
					<span className="lw-admin-sidenav__label">
						{ __( 'Documentation', 'lw-translate' ) }
					</span>
					<Icon icon={ external } size={ 16 } />
					<span className="screen-reader-text">
						{ __( '(opens in a new tab)', 'lw-translate' ) }
					</span>
				</a>
			</div>
		</aside>
